SELECT ROW_NUMBER() OVER (ORDER BY pt.name) as "Sl No",sl.name as Location,pt.name as Product_Name,pc.name as Category,
...
GROUP BY sl.name,pt.name,srp.product_id,pc.name
HAVING 
	SUM(CASE WHEN srp.qty<0 then '0.000'
    ELSE CAST((srp.qty) AS DECIMAL(10,3))
	END) = 0
