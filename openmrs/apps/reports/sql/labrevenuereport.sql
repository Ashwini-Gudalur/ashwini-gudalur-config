...
else 'Tribe' END) as Caste,sum(vop.amount_total) as "Amt to be collect",
sum(vop.amount_tax) as "Amount_Tax",
sum(vop.amount_untaxed) as "Actuall bill without tax",sum(vop.discount_amount) as Discount,
