SELECT ROW_NUMBER() OVER (ORDER BY rp.ref) as "Sl No",rp.ref,so.dispensed_location,so.create_date,rp.name,so.name as "Sale order Number",
...
WHERE so.id in (SELECT distinct(sol.order_id) FROM sale_order_line sol
  WHERE product_id IN(SELECT pp.id FROM product_product pp INNER JOIN product_template pt ON pp.product_tmpl_id = pt.id and pt.categ_id = 535))
and so.shop_id in (8,14)
